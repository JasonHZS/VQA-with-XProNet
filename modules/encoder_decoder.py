...
import pickle
from modules.utils import my_con_loss
...
class EncoderDecoder(AttModel):
    ...
    def __init__(self, args, tokenizer, mode = 'train'):
        ...
        # protypes 初始化涉及到从预训练模型加载的原型数据，这些原型能够为跨模态学习提供先验的知识。
    ...
    def _prepare_feature_forward(self, att_feats, att_masks=None, seq=None, labels=None,):
        ...
        input:
        att_feats: 特征张量，通常是从图像中提取的特征，它的形状是 [batch_size, num_features, feature_dim]。
        att_masks (可选): 一个用于指示有效特征的掩码，形状通常是 [batch_size, num_features]。这个掩码标识出哪些特征是有意义的，哪些是填充的。
        seq (可选): 目标序列，即文本数据。这在训练解码器时使用，形状为 [batch_size, sequence_length]。
        labels (可选): 标签信息，用于从预定义的原型集中选择相关的原型。形状通常是 [batch_size, num_categories]，其中每个元素标记是否属于特定的类别。

        output:
        att_feats: 修改后的特征张量，可能包括应用原型查询和响应后的结果。
        seq: 如果提供了 seq 输入，则该输出是未修改的 seq 输入，因为它在该方法内部并没有被更改。
        att_masks: 与输入相同，除非在函数中对其进行了修改或调整。
        seq_mask: 生成的序列掩码，用于训练时指示有效的序列元素。它是基于 seq 输入生成的，并应用了随后掩码以保持解码器的自回归属性。
        query_matrix: 根据 labels 生成的查询矩阵，它将用于查询跨模态原型矩阵中的相关原型。
        cmn_masks: 为交叉模态原型查询生成的掩码，形状为 [batch_size, sequence_length, max_num_protype]。这个掩码指示了哪些原型是在特定时间步有效的。
        responses: 从交叉模态原型矩阵中查询得到的响应，形状与 att_feats 相似，表示通过原型响应增强的特征。
        """
